...
const Domain: React.FC = () => {
  ...
  // ambil semua domains dari sampleData
  const [domainList, setDomainList] = useState(
    sampleData.flatMap((org) =>
      org.domains.map((d) => ({
        ...
        domain: d.domain,
        package: d.package,
        domMailboxCount: d.domMailboxCount,
        domStorageUsed: d.domStorageUsed,
  ...
  const filteredDomains = domainList
  .filter((d) => !selectedOrg || d.organization === selectedOrg) // filter by org
  .filter((d) => {
    const term = searchTerm.toLowerCase();
    return (
      d.domain.toLowerCase().includes(term) ||
      d.organization.toLowerCase().includes(term) ||
      d.package.toLowerCase().includes(term) ||
      d.domMailboxCount.toString().includes(term) ||
      d.domStorageUsed.toLowerCase().includes(term) ||
      d.domStatus.toLowerCase().includes(term) ||
      d.expiryDate.toLowerCase().includes(term)
    );
  });
  //////////////Search Function ?///////////////
    ...
    // const filteredDomains = selectedOrg
    // ? domainList.filter((d) => d.organization === selectedOrg)
    // : domainList;
  ...
  const currentItems = filteredDomains.slice(startIndex, startIndex + itemsPerPage);
  ...
  const handleAddDomain = () => {
    ...
    const newDomain = {
      ...
      domMailboxCount: formData.domMailboxCount,
      domStorageUsed: formData.domStorageUsed,
      domStatus: formData.domStatus,
    ...
    <div className="mt-6 bg-white rounded-md flex items-center h-10 shadow-sm px-3">
            ...
            {currentItems.length > 0 ? (
              currentItems.map((d) => (
                <tr key={d.id} className=" hover:bg-gray-50">
                  ...
                  <td>{d.package}</td>
                  <td>{d.domMailboxCount}</td>
                  <td>{d.domStorageUsed}</td>
                  <td>{d.domStatus}</td>
                  <td>{d.expiryDate}</td>
                  <td className="py-2">
      ...
      <Popup
        ...
        footer={
          <div className="flex justify-end gap-2">
            ...
            <Button size="sm" variant="primary" onClick={handleAddDomain}>
              Save
            </Button>
          </div>
        }
      >
        <div className="flex flex-col gap-3">
          ...
          <input
            type="text"
            placeholder="Mailbox Total"
            value={formData.domMailboxCount}
            onChange={(e) => setFormData({ ...formData, domMailboxCount: e.target.value })}
            className="border rounded p-2 text-sm"
          />
          <input
            type="text"
            placeholder="Storage Used"
            value={formData.domStorageUsed}
            onChange={(e) => setFormData({ ...formData, domStorageUsed: e.target.value })}
            className="border rounded p-2 text-sm"
          />
          <select
            value={formData.domStatus}
            onChange={(e) => setFormData({ ...formData, domStatus: e.target.value })}
            className="border rounded p-2 text-sm"
          >
