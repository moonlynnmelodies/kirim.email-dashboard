...
const Mailbox: React.FC = () => {
  ...
  // flatten mailbox data dari sampleData
  const [mailboxList, setMailboxList] = useState(
    sampleData.flatMap((org) =>
      org.domains.map((d) => ({
        ...
        status: d.domStatus ?? "Unknown",
        mailStorageUsed: d.mailStorageUsed ?? "-",
        package: d.package ?? "-",
        lastPayment: d.lastPayment ?? "-",   // <-- default "-"
        nextBilling: d.nextBilling ?? "-",   // <-- default "-"
      }))
    )
  );
  ...
  /////////////Search Function////////////////
  const filteredMailboxes = mailboxList
  .filter((m) => !selectedOrg || m.organization === selectedOrg) // filter by organization
  .filter((m) => {
    const term = searchTerm.toLowerCase();
    return (
      (m.mailbox?.toLowerCase() ?? "").includes(term) ||
      (m.domain?.toLowerCase() ?? "").includes(term) ||
      (m.organization?.toLowerCase() ?? "").includes(term) ||
      (m.status?.toLowerCase() ?? "").includes(term) ||
      (m.mailStorageUsed?.toLowerCase() ?? "").includes(term) ||
      (m.package?.toLowerCase() ?? "").includes(term) ||
      (m.lastPayment?.toLowerCase() ?? "").includes(term) ||
      (m.nextBilling?.toLowerCase() ?? "").includes(term)
    );
  });
  //////////////Search Function////////////////
  ...
  const totalItems = filteredMailboxes.length;
  const totalPages = Math.ceil(totalItems / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const currentItems = filteredMailboxes.slice(startIndex, startIndex + itemsPerPage);
  ...
  const handleAddMailbox = () => {
    if (!formData.mailbox.trim()) return;
      ...
      <div className="mt-7 bg-white p-4 rounded-md shadow-sm">
        <table className="table-auto w-full border-separate border-spacing-y-2 px-4">
          <thead>
            <tr>
              <td colSpan={9}>
                <h1 className="text-2xl font-bold mb-5">Mailboxes</h1>
                <div className="flex justify-between mb-6">
                  <p className="text-gray-600 text-sm">Manage mailbox accounts.</p>
            ...
            {currentItems.length > 0 ? (
              currentItems.map((m) => (
                <tr key={m.id} className="hover:bg-gray-50">
                  <td className="px-4 py-2">{m.mailbox}</td>
                  <td>{m.domain}</td>
                  <td>{m.organization}</td>
                  <td>{m.status}</td>
                  <td>{m.mailStorageUsed}</td>
                  <td>{m.package}</td>
                  <td>{m.lastPayment}</td>
                  <td>{m.nextBilling}</td>
                  <td>
                    <div className="flex gap-2">
                      <Link to={`/mailbox/${m.id}`}>
                        <button className="text-blue-600 p-1 rounded hover:bg-gray-200 text-sm">
                      ...
                      <button className="text-green-500 p-1 rounded hover:bg-gray-200 text-sm">
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                            viewBox="0 0 24 24"
                      ...
                      <button className="text-red-500 p-1 rounded hover:bg-gray-200 text-sm">
      ...
      <Popup
        ...
        footer={
          <div className="flex justify-end gap-2">
            ...
            <Button size="sm" variant="primary" onClick={handleAddMailbox}>
              Save
            </Button>
          </div>
        }
      >
        <div className="flex flex-col gap-3">
          ...
          <input
            ...
            value={formData.mailStorageUsed}
            onChange={(e) => setFormData({ ...formData, mailStorageUsed: e.target.value })}
            className="border rounded p-2 text-sm"
          />
          <input
            type="text"
